...
int main() {
	...
	int current_part = 0;
	std::vector<int> shipyard = {};
	bool in_shipyard = true;
	int index = 0;
	for (int i = 0; i < 25 * 25; i++) {
		shipyard.push_back(-1);
	}
    ...
    while (window.isOpen()) {
			...
			if (event.type == sf::Event::Closed) {
				...
				if (in_shipyard) {
					if (event.key.code == sf::Keyboard::W) {
						shipyard[index] = parts[shipyard[index] + 1];
					}
					else if (event.key.code == sf::Keyboard::S) {
						shipyard[index] = parts[shipyard[index].index] - 1;
					}
					if (shipyard[index].index != -1 && shipyard[index].index >= parts.size()) {
						shipyard[index] = -1;
					}
					if (shipyard[index].index < -1) {
						shipyard[index].index = parts.size() - 1;
					}
				}
			}
        }
		...
		if (in_shipyard) {
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i] > -1) {
					drawSprite(parts[shipyard[i]].sX, parts[shipyard[i]].sY, 16, 16, 50 + (i % 25) * 16*4, 50 + (i / 25) * 16 * 4, 4);
				}
				if (i == index) {
					drawSprite(64, 0, 16, 16, 50 + (i % 25) * 16 * 4, 50 + (i / 25) * 16 * 4, 4);
				}
			}
		}
		else {
