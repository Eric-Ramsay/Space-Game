	...
	if (command > -1) { //1 command module, check if everything is connected to it
		...
		while (open_list.size() > 0) {
			...
			for (int i = 0; i < 4; i++) {
				int cons = 0;
				switch (i) {
				case 0: i = safeC(current + 18, shipyard.size()); break;
				case 1: i = safeC(current + 18, shipyard.size()); break;
				case 2: i = safeC(current + 18, shipyard.size()); break;
				case 3: i = safeC(current + 18, shipyard.size()); break;
				}
			}
			if (shipyard[safeC(current + 18, shipyard.size())].index > 0 && !shipyard[safeC(current + 18, shipyard.size())].valid) { //current part is above considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current + 18, shipyard.size())], 0)) {
					open_list.push_back(safeC(current + 18));
				}
			}
			if (shipyard[safeC(current - 1, shipyard.size())].index > 0 && !shipyard[safeC(current - 1, shipyard.size())].valid) { //current part is to the right of considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current - 1, shipyard.size())], 1)) {
					open_list.push_back(safeC(current - 1));
				}
			}
			if (shipyard[safeC(current - 18, shipyard.size())].index > 0 && !shipyard[safeC(current - 18, shipyard.size())].valid) { //current part is below considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current - 18, shipyard.size())], 2)) {
					open_list.push_back(safeC(current - 18));
				}
			}
			if (shipyard[safeC(current + 1, shipyard.size())].index > 0 && !shipyard[safeC(current + 1, shipyard.size())].valid) { //current part is to the left of considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current + 1, shipyard.size())], 3)) {
					open_list.push_back(safeC(current + 1));
				}
			}
		}
