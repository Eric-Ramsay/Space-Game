	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				Print(to_str((rad(180) - abs(a - a1))/rad(180)), player->pos.x - 300, player->pos.y - 150);
				Print(to_str(cos((rad(180) - abs(a - a1)))), player->pos.x - 300, player->pos.y - 100);
				Print(to_str(sin(a - a1)), player->pos.x - 300, player->pos.y - 50);
				Print(to_str(tan(a - a1)), player->pos.x - 300, player->pos.y - 0);
			}
		}
		window.display();
	}
