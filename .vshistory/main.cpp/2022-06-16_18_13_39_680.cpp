...
int main() {
	...
	std::vector<Planet> planets = {plan};
	...
	while (window.isOpen()) {
		...
		if (in_shipyard) {
			...
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i].i > 0) {
					...
					drawSprite(parts[shipyard[i].i].sX + 16 * (shipyard[i].rotation % 2), shipyard[i].sY + 16 * (shipyard[i].rotation / 2), 16, 16, dX + (i % 18) * 16, dY + (i / 18) * 16, 1, 0.0f, color);
				}
				if (i == index) {
				...
				if (s == player) {
					for (int i = -25; i < 25; i++) {
						for (int j = -30; j < 30; j++) {
							bool draw = true;
							for (Part& p : player->parts) {
								if (overlap(partX(s->pos, p), partY(s->pos, p), partW(p), partH(p), s->pos.a, s->pos.x + i * 2, s->pos.y + j * 2)) {
									draw = false;
									break;
								}
							}
							if (draw) {
								drawPosition(s->pos.x + i * 2, s->pos.y + j * 2);
							}
						}
					}
				}
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				for (Part p : player->parts) {
					if (overlap(player->pos, p, mX, mY, 1, 1)){
						Print("Overlap", view.getCenter().x, view.getCenter().y - view.getSize().y / 2.0 + 40, 1, true);
					}
				}
				drawPosition(mX, mY);
			}
