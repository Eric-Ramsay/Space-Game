...
void dock(Ship* ship, Ship* station) {
	...
	Position pos;
	pos.x = half(station->pos.x, ship->pos.x, station->mass, ship->mass);
	pos.y = half(station->pos.y, ship->pos.y, station->mass, ship->mass);
	pos.da = half(station->pos.da, ship->pos.da, station->mass, ship->mass);
	pos.dx = half(station->pos.dx, ship->pos.dx, station->mass, ship->mass);
	pos.dy = half(station->pos.dy, ship->pos.dy, station->mass, ship->mass);
...
void dockPos(Ship* ship) {
	float mass = s->mass;
	float dx = s->pos.dx * mass;
	float dy = s->pos.dy * mass;
	float da = s->pos.da * mass;
	for (Ship* s : s->connected) {
		dx += s->pos.dx * s->mass;
	}
