...
void force(Ship* s, Position f) {
	...
	float x_force = sin(angle) * sin(angle) * min(100, dist(f.x, f.y, s->pos.x, s->pos.y));
	float y_force = cos(angle) * cos(angle) * min(100, dist(f.x, f.y, s->pos.x, s->pos.y));
...
void collide(Ship* a, Ship* b) {
	/*//Force of a on b
	//Counterforce of a on a
	//Force of b on a
	//Counterforce of b on b
	std::vector<Position> forces = {a->pos, b->pos, b->pos, a->pos};
	for (int i = 0; i < 4; i++) {
		int m = 1;
		if (i % 2 == 1) {
			m = -1;
		...
		if (i < 2) {
			forces[i].dx = .5 * a->pos.dx * m * a->mass;
			forces[i].dy = .5 * a->pos.dy * m * a->mass;
			forces[i].a = a->pos.a * (i % 2) * rad(180);
		}
		else {
			forces[i].dx = .5 * b->pos.dx * m * b->mass;
			forces[i].dy = .5 * b->pos.dy * m * b->mass;
			forces[i].a = b->pos.a * (i % 2) * rad(180);
		}
	}
	force(a, forces[1]);
	force(a, forces[2]);
