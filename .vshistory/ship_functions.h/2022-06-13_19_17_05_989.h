...
float calcV(Position p, float x, float y, int iter = 50) {
	for (int i = 0; i < iter; i++) {
		updatePosition(p);
	}
	return dist(x, y, p.x, p.y);
}
float calcV2(Position p, float x, float y, int iter = 50) {
	Position copy = p;
	updatePosition(copy);
	float x_diff = p.x - copy.x;
	float y_diff = p.y - copy.y;
	return dist(x, y, p.x + iter * x_diff, p.y + iter * y_diff);
}
