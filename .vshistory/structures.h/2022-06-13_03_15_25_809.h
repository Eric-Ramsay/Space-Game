...
struct PathPos {
	...
	bool operator ==(PathPos rhs) {
		bool ape = abs(pos.x - rhs.pos.x) < .01 && abs(pos.y - rhs.pos.y) < .01;
		bool ret;
		if (pos.x > rhs.pos.x) {
			ret = (pos.x - rhs.pos.x) < .01;
		}
		else {
			ret = (rhs.pos.x - pos.x) < .01;
		}
		if (pos.y > rhs.pos.y) {
			ret = ret && ((pos.y - rhs.pos.y) < .01);
		}
		else {
			ret = ret && ((rhs.pos.y - rhs.pos.y) < .01);
		}
		if (ret != ape) {
			float dog = 3;
		}
