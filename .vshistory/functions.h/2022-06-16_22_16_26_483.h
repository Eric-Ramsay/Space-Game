...
Rect createRect(Part p, Position ship) {
	return Rect(tl(tLX(p, ship), tLY(p, ship)), Point tr(tRX(p, ship), tRY(p, ship)), Point bl(bLX(p, ship), bLY(p, ship)), Point br(bRX(p, ship), bRY(p, ship)));
}
...
bool bounded(Part p, Position ship, float x, float y, float w = 1, float h = 1) {
	...
	return x >= (x_min - w) && x <= (x_max + w) && y >= (y_min - h) && y <= (y_max + h);
}
