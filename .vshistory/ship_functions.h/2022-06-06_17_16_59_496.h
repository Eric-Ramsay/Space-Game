...
void force(Ship* s, Position f) {
	...
	float angle = atan2(s->pos.y - f.y, s->pos.x - f.x) - (s->pos.a);
	float x_force = -sin(angle) * dist(f.x, f.y, s->pos.x, s->pos.y);
	float y_force = -cos(angle) * dist(f.x, f.y, s->pos.x, s->pos.y);
	T += (x_force * x_force) * f.dx;
	T += (y_force * y_force) * f.dy;
	if (abs(T) > 1000) {
		da += T / (50000 * s->e_mass);
	}
	if ((da > 0 && s->pos.da < 0) || (da < 0 && s->pos.da > 0)) {
		da *= 2;
	}
	s->pos.da += da;
}
...
void collide(Ship* a, Ship* b) {
	...
	force(b, a_force);
	a_force.dx = -a_force.dx;
	a_force.dy = -a_force.dy;
	force(a, a_force);

	/*float angle = -rad(90) + atan2(mY - player->y, mX - player->x) - player->pos.a;
	player->pos.dx = sin(angle);
	player->pos.dy = cos(angle);*/
	...
	float T = (x_force * x_force) * dxt + (y_force * y_force) * dyt;
	if (abs(T) > 1000) {
		da += T / (50000 * s->e_mass);
	}*/
	...
	for (Part& p : s->parts) {
		...
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				...
				Position thrust;
				thrust.a = p.pos.a;
				//These values represent the angle of the thrusters relative to the angle of the ship, no matter what orientation the ship has
				float dX = -cos((PI / 2 * (1 + p.rotation))) * p.thrust;
				float dY = sin((PI / 2 * (1 + p.rotation))) * p.thrust;
				//However, the force function requires a frame of reference where -y is up, and -x is left, so this conversion must be made
				thrust.dx = sin(thrust.a) * thrust.dy + cos(thrust.a) * thrust.dx;
				thrust.dy = sin(thrust.a) * thrust.dx - cos(thrust.a) * thrust.dy;
				thrust.x = p.pos.x;
				thrust.y = p.pos.y;
				force(s, thrust);
				//da += torque;
				...
				float angular_thrust = PI / 2 * (1 + p.rotation);
				angular_thrust = cos(angular_thrust) * p.thrust + sin(angular_thrust) * p.thrust;
				float angle = atan2(p.pos.y - s->pos.y, p.pos.x - s->pos.x) - p.pos.a;
				float x_force = -sin(angle) * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y);
				float y_force = -cos(angle) * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y);
