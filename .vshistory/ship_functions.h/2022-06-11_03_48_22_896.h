...
void collide(Ship* a, Ship* b) {
	...
	collisions++;
	std::cout << "Collided " << collisions << std::endl;
	...
	//float a_Angle = atan2(aF.dy + .01, aF.dx + .01);
	//float b_Angle = atan2(bF.dy + .01, bF.dx + .01);
	...
	std::cout << "BEFORE IMPACT--------------------" << std::endll
	std::cout << "A DX: " << a->pos.dx << std::endl;
	std::cout << "B DX: " << b->pos.dx << std::endl;
...
bool will_collide(Ship a, Ship b, int steps = 0) {
