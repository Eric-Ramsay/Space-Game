    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				...
				float vx = view.getCenter().x;
				float wid = view.getSize().x / 2.0;
				float mx = sf::Mouse::getPosition().x;
				float topleft_x = vx - wid;

				mX = topleft_x + mx * zoom;
				
				mX = view.getCenter().x + sf::Mouse::getPosition().x * zoom - view.getSize().x / 2;
				mY = view.getCenter().y + sf::Mouse::getPosition().y * zoom - view.getSize().y / 2;
			}
				...
				if (event.mouseButton.button == sf::Mouse::Left) {
					...
					mouseForce.x = mX;
					mouseForce.y = mY;
					mouseForce.a = player->pos.a;
					float angle = atan2(mY - player->y, mX - player->x);
					
					mouseForce.dx = cos(mouseForce.a) * 5;
					mouseForce.dy = 0;
		...
		if (player != nullptr && !in_shipyard) {
			float dx = sin(player->pos.a) * player->pos.dy;
			float dy = cos(player->pos.a) * player->pos.dy;
			dx += cos(player->pos.a) * player->pos.dx;
			dy += sin(player->pos.a) * player->pos.dx;
			Print(to_str((int)(100 * dx)), player->x - 50, player->y);
			Print(to_str((int)(100 * dy)), player->x + 50, player->y);
