...
void dock(Ship* a, Ship* b) {
	std::vector<Ship*> ships = b->docked;
	ships.push_back(b);
	for (Ship* s : ships) {
		if (s != a) {
			bool found = false;
			for (Ship* check : a->docked) {
				if (s == check) {
					found = true;
				}
			}
			if (!found) {
				a->docked.push_back(s);
			}
		}
	}
}
...
void collide(Ship* a, Ship* b) {
	std::vector<Ship*> ships = b->collided;
	ships.push_back(b);
	for (Ship* s : ships) {
		if (s != a) {
			bool found = false;
			for (Ship* check : a->collided) {
				if (s == check) {
					found = true;
				}
			}
			if (!found) {
				a->collided.push_back(s);
			}
		}
	}
}
...
void dockPos(Ship* ship) {
	...
	for (Ship* s : ship->collided) {
		float a = s->pos.a - ship->pos.a;
		float delta_x = sin(a) * s->pos.dy * s->mass + cos(a) * s->pos.dx * s->mass;
		float delta_y = cos(a) * s->pos.dy * s->mass - sin(a) * s->pos.dx * s->mass;
		
		if (delta_x > 0 && ) { //Force in the positive x direction
