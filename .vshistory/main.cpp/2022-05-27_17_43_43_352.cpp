...
void drawSprite(int sX, int sY, int w, int h, int dX, int dY, int scale = 1, double rotation = 0.0f, sf::Color color = sf::Color(255, 255, 255)) {
...
double safeC(double a, double MAX) {
	if (a >= MAX) {
		return fmod(a, MAX);
	}
	else if (a < 0) {
...
int Print(std::string text, int x = -1, int y = -1, double scale = 1.0f, bool center = false, int width = 1920 / 2) {
	int cap = width;
	int TEXT_W = 5;
	int TEXT_H = 7;
...
struct Part {
	...
	Part(std::string n = "Nothing", int dura = 25, double m1 = 25, Connection c = Connection(true, false, false, false), bool cruc = false, double t1 = 0) {
		name = n;
		durability = dura;
		connects = c;
		crucial = cruc;
...
void moveShip(Ship* s, bool up, bool down, bool left, bool right) {
	...
	for (Part& p : s->parts) {
		...
		if (p.thrust > 0 && (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left)) {
			...
			double angle = atan2(p.y - my, p.x - mx) - p.angle;
			double x_force = -sin(angle) * dxt * dist(p.x, p.y, mx, my)/2;
			double y_force = -cos(angle) * dyt * dist(p.x, p.y, mx, my)/2;
			T += x_force;
			T += y_force;
			da += T / (5000 * mass);
			dx += dxt;
...
void drawStars(double x, double y, double w, double h) {
	int dx = x - w/2 - 1;
	int dy = y - h/2 - 1;
