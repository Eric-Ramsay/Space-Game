		...
		if (in_shipyard) {
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i].index > 0) {
					...
					else if (shipyard[i].connects.bottom && shipyard[safeC(i + 18, shipyard.size())].index > 0 && shipyard[safeC(i + 18, shipyard.size())].connects.bottom) {
					
					}
					drawSprite(shipyard[i].sX + 16 * (shipyard[i].rotation % 2), shipyard[i].sY + 16 * (shipyard[i].rotation / 2), 16, 16, 50 + (i % 18) * 16*4, 50 + (i / 18) * 16 * 4, 4);
				}
				if (i == index) {
