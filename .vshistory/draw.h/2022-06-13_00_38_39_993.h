...
void drawShip(Ship* s) {
	...
	for (int i = 0; i < s->parts.size(); i++) {
		...
		//float dX = s->pos.x + p.pos.x;
		//float dY = s->pos.y + p.pos.y;
		float co = cos(p.pos.da);
		float si = sin(p.pos.da);
		float x_dif = p.pos.x - s->pos.x;
		float y_dif = p.pos.y - s->pos.y;
		dX = co * x_dif - si * y_dif + s->pos.x;
		dY = si * x_dif + co * y_dif + s->pos.y;
		drawSprite(sX, p.sY + 16 * (p.rotation / 2), 16, 16, , , 1, deg(s->pos.a));
		if (p.turret) {
