...
int Print(std::string text, float x = -1, float y = -1, float scale = 1.0f, bool center = true, int width = 1920 / 2) {
	int cap = width;
	int TEXT_W = 5;
	int TEXT_H = 7;
...
void drawPlanet(Planet* p, sf::View& view) {
	...
	space.setUniform("offset", sf::Glsl::Vec2({ view.getCenter() }));
	space.setUniform("height", p->size);
