...
void moveShip(Ship* s, bool up, bool down, bool left, bool right) {
	...
	for (Part& p : s->parts) {
		...
		float dyt = 0;
		float dxt = 0;
		if (p.thrust > 0 && (p.rotation == 0 && down || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left)) {
			//da += torque;
			switch (p.rotation) {
			...
			float x_force = -sin(angle) * dxt * dist(p.x, p.y, mx, my);
			float y_force = cos(angle) * dyt * dist(p.x, p.y, mx, my);
			T += x_force;
			T += y_force;
			da += T / (100 * mass);
			dx += dxt;
