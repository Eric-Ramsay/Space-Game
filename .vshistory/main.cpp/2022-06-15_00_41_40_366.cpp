...
int main() {
	...
	for (int i = 0; i < 200; i++) {
		int x = 4 * (rand() % 32000) - 64000;
		int y = 4 * (rand() % 32000) - 64000;
		if (dist(x, y, 0, 0) < min) {
			min = dist(x, y, 0, 0);
			star_track_POS.x = x;
			star_track_POS.y = y;
				...
				if (in_shipyard) {
					...
					if (event.key.code == sf::Keyboard::Enter) {
						if (valid_design(shipyard)) {
							...
							if (player == nullptr) {
								player = ship;
								Track t("Nearest Star", star_track_POS);
								player->HUD.push_back(Track);
							}
						}
					}
				}
		...
		if (player != nullptr) {
			if (player->HUD.size() > 0) {
				for (Track t : player->HUD) {
					...
					x += cos(angle + rad(90)) * view.getSize()*x/2.1;
					y += sin(angle + rad(90)) * view.getSize().y/2.1;
					Print(t.msg, x, y);
				}
			}
