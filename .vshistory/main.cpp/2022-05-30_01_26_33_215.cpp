...
struct Position {
	...
	Position(float x1, float y1, float a1, float dx1, float dy1, float da1) {
		x = x1;
		y = y1;
		a = a1;
		dx = dx1;
...
struct Bullet {
	...
	Bullet(float x1, float y1, float dx1, float dy1, float da1, float p = 1000, sf::Color c = sf::Color(255, 0, 0)) {
		x = x1;
		y = y1;
		angle = a;
		velocity = v;
...
void applyGravity(Ship* s, Planet plan) {
	...
	if (d <= 20 * plan.size) {
		...
		for (Part& p : s->parts) {
			p.dx += dx * -cos(angle - p.angle);
			p.dy += dy * sin(angle - p.angle);
		}
	}
}
...
void drawFutureTrajectory(Ship s, Planet plan) {
	Ship copy = s;
...
std::vector<Part> initParts() {
	...
	parts.push_back(Part("Solar Panel", 20, 15, Connection(true, false, false, false)));	
    ...
    while (window.isOpen()) {
		...
		if (elapsedTime >= targetTime) {
			...
			if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
				for (Part p : player.parts) {
					if (p.turret) {
						Bullet* b = new Bullet(p.x, p.y, p.turret_angle + rad(90), hypot(p.dx, p.dy) + 10);
						b->x += cos(b->angle) *6;
						b->y += sin(b->angle) *6;
						b->next = bullets;
						bullets = b;
