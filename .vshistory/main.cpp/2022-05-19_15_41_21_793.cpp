...
void moveShip(Ship* s, int r) {
	...
	for (Part& p : s->parts) {
		...
		float x_d = cos(angle) * d;
		float y_d = sin(angle) * d;
		p.x +=  + ;
		p.y += sin(p.angle) * p.velocity + sin(angle) * d;
...
void drawShip(Ship* s) {
	for (int i = 0; i < s->parts.size(); i++) {
		Part p = s->parts[i];
		drawSprite(p.sX + 16 * (p.rotation % 2), p.sY + 16 * (p.rotation / 2), 16, 16, p.x - (8 * 4 * cos(p.angle)), p.y - (8 * 4 * sin(p.angle)), 4, 90 + deg(p.angle));
	}
}
