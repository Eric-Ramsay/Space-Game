    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				...
				float view.getCenter().x;
				float wid = view.getSize().x / 2.0;
				float mx = sf::Mouse::getPosition().x;
				
				mX = view.getCenter().x + sf::Mouse::getPosition().x * zoom - view.getSize().x / 2 * zoom;
				mY = view.getCenter().y + sf::Mouse::getPosition().y * zoom - view.getSize().y / 2 * zoom;
			}
