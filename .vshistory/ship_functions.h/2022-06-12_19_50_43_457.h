...
int steps_to_reach(Ship s, float x, float y, std::vector<Planet>& plan) {
...
struct PathPos {
	...
	bool left;
	bool right;
	PathPos* parent = nullptr;
	bool operator ==(PathPos rhs) {
		return abs(pos.x - rhs.pos.x) < .1 && abs(pos.y - rhs.pos.y) < .1 && abs(pos.dx - rhs.pos.dx) < .1 && abs(pos.dy - rhs.pos.dy) < .1 && abs(pos.da - rhs.pos.da) < .1;
	}
	PathPos(Position p, PathPos* par, bool u = false, bool d = false, bool l = false, bool r = false) {
		pos = p;
		parent = par;
		up = u;
		down = d;
...
void path(Ship a, float x, float y, bool& up, bool& down, bool& left, bool& right) {
	...
	bool stop = false;
	Position final;
	PathPos current(a.pos, nullptr);
	std::deque<Position> open = {current};
			...
			for (int d = 0; d < 2; d++) {
				for (int l = 0; l < 2; l++) {
					for (int r = 0; r < 2; r++) {
						...
						PathPos ape(copy.pos, &current, u, d, l, r);
