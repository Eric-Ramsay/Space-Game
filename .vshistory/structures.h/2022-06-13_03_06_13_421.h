...
struct PathPos {
	...
	bool operator ==(PathPos rhs) {
		bool ret;
		if (pos.x > rhs.pos.x) {
			ret = pos.x - rhs.pos.x < .001;
		}
		else {
			ret = rhs.pos.x - pos.x < .001;
		}
		if (pos.y > rhs.pos.y) {
			return ret && (pos.y - rhs.pos.y < .001);
		}
		else {
			ret = rhs.pos.y - pos.y < .001;
		}
		return abs(pos.x - rhs.pos.x) < .001 && abs(pos.y - rhs.pos.y) < .001;
	}
