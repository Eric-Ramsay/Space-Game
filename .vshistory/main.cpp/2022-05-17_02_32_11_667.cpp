...
struct Ship {
	...
	int w = 0;
	int h = 0;
	Ship(float x1, float y1, float a, float v, int sx, int sy, int w1, int h1 ) {
		x = x1;
		y = y1;
		angle = a;
		velocity = v;
    ...
    while (window.isOpen()) {
        ...
        if (elapsedTime >= targetTime) {
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
				Ship bullet = Ship(player.x, player.y, player.angle, 50, 224, 16, 4, 1);
				bullet.x = safeC(player.x + cos(rad(player.angle)) * player.velocity, 1920);
				bullet.y = safeC(player.y + sin(rad(player.angle)) * player.velocity, 1080);
			...
			player.x = safeC(player.x + cos(rad(player.angle)) * player.velocity, 1920);
			player.y = safeC(player.y + sin(rad(player.angle)) * player.velocity, 1080);
		...
		for (Ship* s : things) {
			drawSprite(s->sX, s->sY, s->w, s->h, s->x, s->y, 4, s->angle);
		}
