		...
		if (in_shipyard) {
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i].index > 0) {
					...
					drawSprite(shipyard[i].sX + 16 * (shipyard[i].rotation % 2), shipyard[i].sY + 16 * (shipyard[i].rotation / 2), 16, 16, view.getCenter().x - view.getSize().x/2 + (i % 18) * 16, view.getCenter().y - view.getSize().y / 2 + (i / 18) * 16, 1, 0.0f, color);
				}
				if (i == index) {
