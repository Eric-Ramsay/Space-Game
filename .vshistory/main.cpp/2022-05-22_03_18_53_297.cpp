...
void drawStars(float x, float y, float w, float h) {
	...
	int size;
	for (int i = dy; i < dy + w; i++) {
		for (int j = dx; j < dx + h; j++) {
			size = 1 + (int)(randC(i, j) * 5);
			drawSprite(int sX, int sY, int w, int h, int dX, int dY, int scale = 1, float rotation = 0.0f, sf::Color color
				drawSprite(, 0, size, size, dx, dy, size, 0.0f, colors[randC(j, i) * 5]);
				sf::CircleShape shape(randC(j, i) * 8);
...
int main() {
    ...
    while (window.isOpen()) {
		...
		if (in_shipyard) {
			...
			drawStars(view.getCenter().x, view.getCenter().y, view.getCenter().x, view.getCenter().y);
		}
