	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				Print(to_str(posA(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 50);
				Print(to_str(cos(a - a1)), player->pos.x - 200, player->pos.y - 50);
				Print(to_str(sin(a - a1)), player->pos.x - 200, player->pos.y - 50);
			}
		}
		window.display();
	}
