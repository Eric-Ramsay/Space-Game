...
void collide(Ship* a, Ship* b) {
	...
	float aM = sqrtf(a->pos.dx * a->pos.dx + a->pos.dy * a->pos.dy)
	float bM = sqrtf(b->pos.dx * b->pos.dx + b->pos.dy * b->pos.dy)

	float pos.dxa = maga * cos(anglea - collisionAngle)
	float pos.dya = maga * sin(anglea - collisionAngle)
	float pos.dxb = magb * cos(angleb - collisionAngle)
	float pos.dyb = magb * sin(angleb - collisionAngle)

	float finalpos.dxa = ((a->mass - b->mass) * pos.dxa + (b->mass + b->mass) * pos.dxb) / (a->mass + b->mass)
	float finalpos.dxb = ((a->mass + a->mass) * pos.dxa + (b->mass - a->mass) * pos.dxb) / (a->mass + b->mass)
	float finalpos.dya = pos.dya
	float finalpos.dyb = pos.dyb
