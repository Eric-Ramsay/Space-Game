	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				drawPosition(mX, mY, 3, 0, sf::Color::Red);
				for (int i = 0; i < 10; i++) {
					x += cos(a) * 10;
					y += sin(a) * 10;
					drawPosition(x, y);
				...
				Print("X Multiplier: " + to_str(multX(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 75);
				Print("Y Multiplier: " + to_str(multY(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 50);
			}
		}
		window.display();
	}
