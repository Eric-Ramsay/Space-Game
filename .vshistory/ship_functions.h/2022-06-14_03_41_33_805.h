...
float calcV2(Position p, float x, float y, int iter = 60) {
	for (int i = 0; i < 3*iter; i++) {
		updatePosition(p);
	}
	return dist(x, y, p.x, p.y);
}
float calcV(Position p, float x, float y, int iter = 60) {
	Position copy = p;
	updatePosition(copy);
	float dis = dist(x, y, copy.x, copy.y) - dist(x, y, p.x, p.y);
	return 500 * dis;
}
