...
void collide(Ship* a, Ship* b) {
	...
	aF.dx = cos(collision_angle) * cos(pos_angle) * bF.dy;
	aF.dy = sin(collision_angle) * sin(pos_angle) * aF.dy;

	bF.dx = cos(-collision_angle) * cos(-pos_angle) * bF.dy;
	...
	for (Part& p : s->parts) {
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				...
				thrust.dx = -cos((PI / 2 * (1 + p.rotation))) * p.thrust * cos(thrust.a);
				thrust.dy = sin((PI / 2 * (1 + p.rotation))) * p.thrust * sin(thrust.a);
				thrust.x = p.pos.x;
				thrust.y = p.pos.y;
				force(s, thrust);
			}
