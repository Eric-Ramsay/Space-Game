...
void force(Ship* s, Position f) {
	...
	float angle = atan2(s->pos.y - f.y, s->pos.x - f.x) - s->pos.angle;
	float angle2 = deg(atan2(s->pos.y - f.y, s->pos.x - f.x) + s->pos.angle));
	float angle3 = deg(atan2(s->pos.y - f.y, s->pos.x - f.x));
	float angle4 = deg(atan2(s->pos.y - f.y, s->pos.x - f.x) - f.a);
	float angle5 = deg(atan2(s->pos.y - f.y, s->pos.x - f.x) + f.a);
	...
	for (Part& p : s->parts) {
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				p.active = true;
				Position thrust;
				thrust.a = p.pos.a + PI / 2 * (1 + p.rotation);
