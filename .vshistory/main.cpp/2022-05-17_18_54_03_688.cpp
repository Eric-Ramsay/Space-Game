    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				...
				if (in_shipyard) {
					...
					if (shipyard[index] > -1 && shipyard[index] >= parts.size()) {
						shipyard[index] = -1;
					}
					if (shipyard[index] < -1) {
		...
		if (in_shipyard) {
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i] > -1) {
					drawSprite(parts[shipyard[i]].sX, parts[shipyard[i]].sY, 16, 16, 50 + (i % 25) * 18*4, 50 + (i / 25) * 18 * 4, 4);
				}
				if (i == index) {
					drawSprite(64, 0, 16, 16, 50 + (i % 25) * 18 * 4, 50 + (i / 25) * 18 * 4, 4);
				}
			}
		}
		else {
