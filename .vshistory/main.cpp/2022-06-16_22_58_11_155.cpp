				...
				if (s == player) {
					/*for (int i = -50; i < 50; i++) {
						for (int j = -60; j < 60; j++) {
							bool draw = true;
							for (Part& p : player->parts) {
								if (bounded(p, s->pos, s->pos.x + i, s->pos.y + j, 1, 1)) {
									draw = false;
									break;
								}
							}
							if (draw) {
								drawPosition(s->pos.x + i, s->pos.y + j);
							}
						}
					}*/
				}
			...
			if (!in_shipyard) {
				for (Part p : player->parts) {
					if (parts[p.i].crucial) {
						Rect part = createRect(p, player->pos);
						float x_min = minX(part);
						float y_min = minY(part);
						float x_max = maxX(part);
						float y_max = maxY(part);
						Print("X Min: " + to_str(((int)(x_min * 10)) / 10.0), player->pos.x - 50, player->pos.y - 25);
						Print("Y Min: " + to_str(((int)(y_min * 10)) / 10.0), player->pos.x - 50, player->pos.y + 25);
						Print("X Max: " + to_str(((int)(x_max * 10)) / 10.0), player->pos.x + 50, player->pos.y - 25);
						Print("Y Max: " + to_str(((int)(y_max * 10)) / 10.0), player->pos.x + 50, player->pos.y + 25);
					}
				}
				Print(to_str(((int)(mX * 10)) / 10.0), mX, mY - 10);
				Print(to_str(((int)(mY * 10)) / 10.0), mX, mY + 10);
