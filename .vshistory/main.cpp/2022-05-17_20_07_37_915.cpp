...
Connection rotConnect(Connection co, rotation r) {
	Connection c = co;
...
bool connectedParts(Part a, Part b, int relation) {
	Connection a = rotConnect(a.connects, a.rotation);
	Connection b = rotConnect(b.connects, b.rotation);
...
bool valid_design(std::vector<Part>& shipyard) {
	...
	if (command > -1) { //1 command module, check if everything is connected to it
		...
		while (open_list.size() > 0) {
			...
			if (shipyard[safeC(current + 18)].index > 0 && !shipyard[safeC(current + 18)].valid) { //current part is above considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current + 18)], 0)) {
					open_list.push_back(safeC(current + 18));
				}
			}
			if (shipyard[safeC(current - 1)].index > 0 && !shipyard[safeC(current - 1)].valid) { //current part is to the right of considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current - 1)], 1)) {
					open_list.push_back(safeC(current - 1));
				}
			}
			if (shipyard[safeC(current - 18)].index > 0 && !shipyard[safeC(current - 18)].valid) { //current part is below considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current - 18)], 2)) {
					open_list.push_back(safeC(current - 18));
				}
			}
			if (shipyard[safeC(current + 1)].index > 0 && !shipyard[safeC(current + 1)].valid) { //current part is to the left of considered part
				if (connectedParts(shipyard[current], shipyard[safeC(current + 1)], 3)) {
					open_list.push_back(safeC(current + 1));
				}
			}
		}
