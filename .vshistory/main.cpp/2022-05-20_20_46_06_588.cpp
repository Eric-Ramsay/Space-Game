...
void moveShip(Ship* s, int r) {
	...
	for (Part& p : s->parts) {
		if (p.thrust > 0 && p.rotation == r) {
			float angle_dir = 0;
			//Calculate Torque
			float torque = dist(p.x, p.y, mx, my) * cos(angle) * p.thrust/(mass*mass*100);
			//da += torque;
			switch (r) {
			case 0: dy += p.thrust; break;
			case 1: dx += p.thrust; break;
			case 2: dy -= p.thrust; break;
			case 3: dx -= p.thrust; break;
			}
			float distAngle = atan2(p.y - my, p.x - mx);
			float fireAngle = atan2(dy, dx);
			float theta = fireAngle - distAngle;
			float torque = dist(p.x, p.y, mx, my) * sin(theta);
			da += (angle - dir_angle) / (mass * mass);
		}
	}
	dy /= mass;
	dx /= mass;
    ...
    while (window.isOpen()) {
		...
		if (in_shipyard) {
			...
			for (Ship* s : things) {
				drawShip(s);
				if (s->parts.size() > 0) {
					Print(to_str(dot2(vec2(3, 7), vec2(8, 1))), 1920 / 2, 1080 / 2, 4);
