...
void moveShip(Ship* s, int r) {
	...
	for (Part p : s->parts) {
		mbot += p.mass;
		mx_top += p.mass * (p.x + 8 * 4 * cos(p.angle));
		my_top += p.mass * (p.y + 8 * 4 * sin(p.angle));
	...
	for (Part& p : s->parts) {
		...
		angle = atan2((p.y + 8 * 4 * sin(p.angle)) - my_top, (p.x + 8 * 4 * cos(p.angle)) - mx_top);
		p.angle += .0001;
...
void drawShip(Ship* s) {
	for (int i = 0; i < s->parts.size(); i++) {
		Part p = s->parts[i];
		drawSprite(p.sX + 16 * (p.rotation % 2) + (8 * 4 * cos(p.angle), p.sY + 16 * (p.rotation / 2) + (8 * 4 * sin(p.angle), 16, 16, p.x, p.y, 4, deg(p.angle));
	}
}
