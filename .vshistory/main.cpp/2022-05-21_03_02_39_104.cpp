...
void moveShip(Ship* s, int r) {
	...
	for (Part& p : s->parts) {
		...
		if (p.thrust > 0 && p.rotation == r) {
			//da += torque;
			switch (r) {
			case 0: dy += p.thrust; T = p.thrust * (p.x - mx); break;
			case 1: dx += p.thrust; T = p.thrust * abs(p.y - my); break;
			case 2: dy -= p.thrust; T = -p.thrust * (p.x - mx); break;
			case 3: dx -= p.thrust; T = -p.thrust * abs(p.y - my); break;
			}
			float x_force = dx * cos(p.angle);
			float y_force = dy * sin(p.angle);
			if (x_force > 0) {
