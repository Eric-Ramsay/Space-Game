...
void collide(Ship* a, Ship* b) {
	...
	float ang = a->pos.a - b->pos.a;
	float dx = sin(ang) * b->pos.dy + cos(ang) * b->pos.dx;
	float dy = cos(ang) * b->pos.dy - sin(ang) * b->pos.dx;

	a->pos.dx = dx*b->mass + a->pos.dx*a->mass;
	a->pos.dy = dy*b->mass + a->pos.dy*a->mass;

	a->pos.dx /= (a->mass + b->mass);
	a->pos.dy /= (a->mass + b->mass);
	...
	float xDist = a->x - b->x
	float yDist = a->y - b->y
	float collisionAngle = Math.atan2(yDist, xDist)

	float aM = Math.sqrt(a->xSpeed * a->xSpeed + a->ySpeed * a->ySpeed)
	float bM = Math.sqrt(b->xSpeed * b->xSpeed + b->ySpeed * b->ySpeed)

	float anglea = Math.atan2(a->ySpeed, a->xSpeed)
	float angleb = Math.atan2(b->ySpeed, b->xSpeed)

	float xSpeeda = maga * Math.cos(anglea - collisionAngle)
	float ySpeeda = maga * Math.sin(anglea - collisionAngle)
	float xSpeedb = magb * Math.cos(angleb - collisionAngle)
	float ySpeedb = magb * Math.sin(angleb - collisionAngle)

	float finalxSpeeda = ((a->mass - b->mass) * xSpeeda + (b->mass + b->mass) * xSpeedb) / (a->mass + b->mass)
	float finalxSpeedb = ((a->mass + a->mass) * xSpeeda + (b->mass - a->mass) * xSpeedb) / (a->mass + b->mass)
	float finalySpeeda = ySpeeda
	float finalySpeedb = ySpeedb

	a->xSpeed = Math.cos(collisionAngle) * finalxSpeeda + Math.cos(collisionAngle + Math.PI / 2) * finalySpeeda
	a->ySpeed = Math.sin(collisionAngle) * finalxSpeeda + Math.sin(collisionAngle + Math.PI / 2) * finalySpeeda
	b->xSpeed = Math.cos(collisionAngle) * finalxSpeedb + Math.cos(collisionAngle + Math.PI / 2) * finalySpeedb
	b->ySpeed = Math.sin(collisionAngle) * finalxSpeedb + Math.sin(collisionAngle + Math.PI / 2) * finalySpeedb
	...
	for (Part& p : s->parts) {
		...
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				...
				float x_force = -sin(angle) * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y);
				float y_force = -cos(angle) * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y);
				T += (x_force * x_force) * dxt;
				T += (y_force * y_force) * dyt;
				if (abs(T) > 1000) {
					da += T / (50000 * s->e_mass);
				}
