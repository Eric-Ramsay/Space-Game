...
void collide(Ship* a, Ship* b) {
	...
	float xDist = a->x - b->x
	float yDist = a->y - b->y
	float angle = Math.atan2(yDist, xDist)

	float aM = sqrtf(a->pos.dx * a->pos.dx + a->pos.dy * a->pos.dy)
	float bM = sqrtf(b->pos.dx * b->pos.dx + b->pos.dy * b->pos.dy)

	float pos.dxa = aM * cos(a->pos.a - angle)
	float pos.dya = aM * sin(a->pos.a - angle)
	float pos.dxb = bM * cos(b->pos.a - angle)
	float pos.dyb = bM * sin(b->pos.a - angle)
	...
	a->xSpeed = Math.cos(angle) * finalxSpeeda + Math.cos(angle + Math.PI / 2) * finalySpeeda
	a->ySpeed = Math.sin(angle) * finalxSpeeda + Math.sin(angle + Math.PI / 2) * finalySpeeda
	b->xSpeed = Math.cos(angle) * finalxSpeedb + Math.cos(angle + Math.PI / 2) * finalySpeedb
	b->ySpeed = Math.sin(angle) * finalxSpeedb + Math.sin(angle + Math.PI / 2) * finalySpeedb
