...
bool aDiff(float a1, float a2) {
	return rad(180 - abs(abs(deg(a1 - a2)) - 180))
}
...
void targetGuns(Ship* s, float x, float y) {
	for (Part& p : s->parts) {
		if (p.turret) {
			...
			int turret_angle = angle - rad(90 * p.rotation);

			float diff = aDiff(angle, p.turret_angle);

			if (turret_angle > 330) {
				p.turret_angle = rad(330 + 90 * p.rotation);
			}
			else if (turret_angle < 210) {
				p.turret_angle = rad(210 + 90 * p.rotation);
			}
			else {
				p.turret_angle = ((int)(10 * angle)) / 10.0;
			}
...
int main() {
    ...
    while (window.isOpen()) {
		...
		Print(to_str((int)mX) + " " + to_str((int)mY), view.getCenter().x, view.getCenter().y - view.getSize().y * .4);
		Print(to_str((int)player.cx) + " " + to_str((int)player.cy), view.getCenter().x, view.getCenter().y - view.getSize().y * .3);
        window.display();
    }
