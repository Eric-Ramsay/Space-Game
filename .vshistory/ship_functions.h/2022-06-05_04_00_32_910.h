...
void collide(Ship* a, Ship* b) {
	...
	float a_diff = a->pos.a - angle;
	float b_diff = b->pos.a - angle;
	...
	a->pos.dx = cos(angle) * aX + Math.cos(angle + Math.PI / 2) * finalySpeeda
	a->pos.dy = sin(angle) * aX + Math.sin(angle + Math.PI / 2) * finalySpeeda
	b->pos.dx = cos(angle) * bX + Math.cos(angle + Math.PI / 2) * finalySpeedb
	b->pos.dy = sin(angle) * bX + Math.sin(angle + Math.PI / 2) * finalySpeedb
