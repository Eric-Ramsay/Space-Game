...
void collide(Ship* a, Ship* b) {
	...
	float angle = -rad(90) + atan2(a->pos.y - b->pos.y, a->pos.x - b->pos.x) - (a->pos.a);
	float dx = sin(angle) * b->pos.dx;
	float dy = cos(angle) * b->pos.dx;
	...
	float ny = (b->pos.y - a->pos.x) / d;

	//Tangent
	float tx = -ny;
	float ty = nx;

	Position a_pos = a->pos;
	a_pos.x = 0; a_pos.y = 0;
	updatePosition(a_pos);

	Position b_pos = b->pos;
	b_pos.x = 0; b_pos.y = 0;
	updatePosition(b_pos);
	//a->

	//Dot Product Tangent
	float dpTan1 = a_pos.x * tx + a_pos.y * ty;
	float dpTan2 = b_pos.x * tx + b_pos.y * ty;
