...
void drawExplosion(Explosion* e) {
	...
	sf::Vertex* vertices = new sf::Vertex[n+1];
	for (int i = 0; i < n+1; i++) {
		if (i == n) {
			vertices[i] = sf::Vertex(sf::Vector2f(e->points[0].x, e->points[0].y));
		}
		else {
	...
	// draw it
	window.draw(vertices, n+1, sf::TriangleFan);
}
