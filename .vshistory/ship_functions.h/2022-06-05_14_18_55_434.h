...
void collide(Ship* a, Ship* b) {
	...
	float angle = -rad(90) + atan2(b->pos.y - a->pos.y, b->pos.x - a->pos.x) - (a->pos.a);
	float dx = sin(angle);
	float dy = cos(angle);
...
void force(Ship* s, Position f) {
	...
	float T = 0;
	float da = 0;
	float angle = atan2(f.y - s->pos.y, f.x - s->pos.x) - (f.a);

	s->pos.dx += cos(angle) * f.dx/ s->e_mass;
	s->pos.dy += sin(angle) * f.dy/ s->e_mass;
...
std::vector<Part> initParts() {
	...
	parts.push_back(Part("Large Thruster", 40, 3, Connection(true, false, false, false), false, 40));
	...
	parts.push_back(Part("Small Thruster", 20, 1.5, Connection(true, false, false, false), false, 20));
