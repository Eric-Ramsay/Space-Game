...
void collide(Ship* a, Ship* b) {
	...
	float angle = atan2(a->pos.y - b->pos.y, a->pos.x - b->pos.x);
	float ang = a->pos.a - b->pos.a;
	float dx = sin(angle) * (sin(ang) * b->pos.dy + cos(ang) * b->pos.dx);
	float dy = cos(angle) * (cos(ang) * b->pos.dy - sin(ang) * b->pos.dx);
