...
void applyGravity(Ship* s, Planet plan) {
	...
	if (d <= 20 * plan.size) {
		...
		float dx = -(cos(angle) * (3*plan.size)) / (10 + d*d);
		float dy = (sin(angle) * (3*plan.size)) / (10 + d*d);
		for (Part& p : s->parts) {
			p.dx += dx;
			p.dy += dy;
		}
...
void moveShip(Ship* s, bool up = false, bool down = false, bool left = false, bool right) {
