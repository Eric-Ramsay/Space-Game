...
void dockPos(Ship* ship) {
	double mx = ship->pos.x * ship->mass;
	double my = ship->pos.y * ship->mass;
	float mass = ship->mass;
	float dx = (sin(ship->pos.a) * ship->pos.dy + cos(ship->pos.a) * ship->pos.dx) * ship->mass;
	float dy = (sin(ship->pos.a) * ship->pos.dx - cos(ship->pos.a) * ship->pos.dy) * ship->mass;
	float da = ship->pos.da * mass;
	for (Ship* s : ship->connected) {
		mx += s->pos.x * s->mass;
		my += s->pos.y * s->mass;
		dx += (sin(s->pos.a) * s->pos.dy + cos(s->pos.a) * s->pos.dx) * s->mass;
		dy += (sin(s->pos.a) * s->pos.dx - cos(s->pos.a) * s->pos.dy) * s->mass;
		da += s->pos.da * s->mass;
		mass += s->mass;
	}
	if (mass == 0) {
	...
	for (Ship* s : ship->connected) {
		s->pos.dx = fX(dx, ship->pos.a);
		s->pos.dy = fY(dy, ship->pos.a);
		s->pos.da = da;
		s->x = (float)mx;
		s->y = (float)my;
		s->e_mass = mass;
...
float fX(Position f, float angle) {
	...
	float dx = x * cos(angle) - y * sin(angle);
	
}
float fY(Position pos, float angle) {
