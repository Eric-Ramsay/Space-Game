...
void dockPos(Ship* ship) {
	...
	for (Ship* s : ship->connected) {
		float angle = atan2(player->y - s->y, player->x - mX) - player->parts[0].pos.a;
		dx += sin(s->pos.a) * s->pos.dy * s->mass;
		dy -= cos(s->pos.a) * s->pos.dy * s->mass;
		dx += cos(s->pos.a) * s->pos.dx * s->mass;
		dy += sin(s->pos.a) * s->pos.dx * s->mass;
		da += s->pos.da * s->mass;
		mass += s->mass;
	}
	dx /= mass;
