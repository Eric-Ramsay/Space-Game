...
float calcV(Position p, float x, float y, int it) {
	for (int i = 0; i < it * 20; i++) {
		updatePosition(p);
	}
...
std::deque<PathPos> path(Ship a, float x, float y) {
	...
	while (closed.size() < 300 && open.size() > 0 && !stop) {
		...
		float dis = dist(current.pos.x, current.pos.y, x, y);
		std::cout << "Iteration: " << closed.size() << std::endl;
		std::cout << "Open Size: " << open.size() << std::endl;
		std::cout << "Distance: " << dis << std::endl << std::endl;
		if (dis < closest) {
				...
				for (int d = 0; d < 2; d++) {
					for (int l = 0; l < 2; l++) {
						for (int r = 0; r < 2; r++) {
							...
							for (int i = 0; i < 1 + dis/10; i++) {
								moveShip(&copy, u, d, l, r);
							}
							PathPos p(copy.pos, closed.size() - 1, u, d, l, r);
							//Check in closed list for the current PathPos
							for (PathPos p1 : closed) {
								if (p == p1) {
									found = true;
									break;
								}
							...
							if (!found) {
								//Check in opened list for the current PathPos
								int size = open.size();
								...
								if (!found) {
									p.iterations = 1 + (int)dis/10;
									p.g = 0; // dist(a.pos.x, a.pos.y, ape.pos.x, ape.pos.y);
									p.h = dist(x, y, copy.pos.x, copy.pos.y);
									//p.v = calcV(p.pos, x, y, p.iterations);
									p.f = 0;// (cur_fuel - copy.energy);
									p.h = p.h + p.v + p.f;
									open.push_back(p);
								}
							}
