	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				for (int i = 0; i < 10; i++) {
				Print(to_str(posA(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 125);
				float adir = atan2(player->pos.dy, player->pos.dx);
				float bdir = atan2(bdy, bdx);
				Print(to_str(cos(a - a1)), player->pos.x - 200, player->pos.y - 75);
				Print(to_str(sin(a - a1)), player->pos.x - 200, player->pos.y - 25);
			}
		}
		window.display();
	}
