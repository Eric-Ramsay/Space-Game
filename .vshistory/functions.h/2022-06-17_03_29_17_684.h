...
float LX(Part p, Position ship, bool top = true) {
...
float LY(Part p, Position ship, bool top = true) {
...
float RX(Part p, Position ship, bool top = true) {
	float x = partX(ship, p);
	short w = parts[p.i].w;
	short h = parts[p.i].h;
	if (p.rotation % 2 != 0) {
...
float RY(Part p, Position ship, bool top = true) {
	float y = partY(ship, p);
	short w = parts[p.i].w;
	short h = parts[p.i].h;
	if (p.rotation % 2 != 0) {
