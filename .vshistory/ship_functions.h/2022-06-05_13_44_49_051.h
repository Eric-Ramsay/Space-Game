...
void collide(Ship* a, Ship* b) {
	...
	float phi = atan2(b->pos.y - a->pos.y, b->pos.x - a->pos.x);
	...
	a->pos.dx = (a_dX * cos(tA - phi)  * (aM + bM) * 2 * bM * b_dX * cos(tB - phi))

	a->pos.dx = (a->pos.dx * cos(a->pos.a - phi) * (a->mass - b->mass) + 2 * b->mass * b->pos.dx * cos(b->pos.a - phi))

	float angle = -rad(90) +  - (a->pos.a);
	float dx = .01 * (sin(angle) * (a->pos.dx - b->pos.dx) * b->mass)/(a->mass);
	float dy = .01 * (cos(angle) * (a->pos.dy - b->pos.dy) * b->mass)/(a->mass);
