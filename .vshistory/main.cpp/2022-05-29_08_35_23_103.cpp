...
void drawPlanet(Planet* p) {
	...
	if (init) {
		init = false;
		if (!planet.loadFromFile("space.frag", sf::Shader::Fragment) {
			std::cout << "There was an error loading a fragment shader" << std::endl;
			break;
		}
	}
	space.setUniform("offset", sf::Glsl::Vec2{ p->x, p->y });
	planet.setUniform("offset", sf::Glsl::Vec2(p->x, p->y));
	sf::CircleShape planet(p->size);
	planet.setPosition(p->x, p->y);
	window.draw(planet);
}
