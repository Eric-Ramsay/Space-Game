...
void force(Ship* s, Position f) {
	...
	float d = dist(f.x, f.y, s->pos.x, s->pos.y) / 100;
	float x_force = sin(angle) * sin(angle) * dist(f.x, f.y, s->pos.x, s->pos.y)
	float y_force = cos(angle) * cos(angle) * dist(f.x, f.y, s->pos.x, s->pos.y)
