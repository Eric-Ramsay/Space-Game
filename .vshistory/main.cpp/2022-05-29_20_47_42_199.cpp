...
bool valid_design(std::vector<Part>& shipyard) {
	bool docking_clamps = false;
	bool command_module = false;
	...
	for (int i = 0; i < shipyard.size(); i++) {
		if (shipyard[i].index > 0) {
			if (shipyard[i].crucial) {
				if (shipyard[i].name == "Command Module") {
					if (command_module) {
						return false;
					}
				}
	...
	if (command_module && docking_clamps) { //1 command module, check if everything is connected to it
...
std::vector<Part> initParts() {
	...
	parts.push_back(Part("Docking Clamps", 20, 15, Connection(true, false, true, true)));
