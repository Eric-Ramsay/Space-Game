...
bool future_collision(Ship* a_src, Ship* b_src, int steps = 0) {
	...
	for (int i = 0; i < steps; i++) {
		...
		for (Part pa : a.parts) {
			for (Part pb : b.parts) {
				if (overlap(a_src->pos, b_src->pos, pa, pb)) {
...
std::vector<Part> collision(Ship* a, Ship* b) {
	std::vector<Part> parts = {};
	int max_dist = a->parts.size() + b->parts.size() * 16;
	if (d2(a->pos, b->pos) < max_dist * max_dist) {
		for (Part pa : a->parts) {
			for (Part pb : b->parts) {
				if (overlap(a->pos, b->pos, pa, pb)) {
...
bool connectedParts(Part a1, Part b1, int relation) {
	Connection a = rotConnect(parts[a1.i].connects, parts[a1.i].rotation);
	Connection b = rotConnect(parts[b1.i].connects, parts[b1.i].rotation);
