...
struct Bullet {
	...
	float velocity = 0.0;
	float power = 10;
	Bullet(float x1, float y1, float a, float v, float p = 50, sf::Color c = sf::Color(255, 0, 0)) {
		x = x1;
		y = y1;
		angle = a;
		velocity = v;
...
void targetGuns(Ship* s, float x, float y) {
	for (Part& p : s->parts) {
		if (p.turret) {
			...
			float turret_angle = (int)deg(angle - rad(90 * p.rotation));
			angle = ((int)(10 * angle)) / 10.0;
			if (turret_angle > 210 && turret_angle < 330) {
				p.turret_angle = angle;
			}
			//I should add a thing so the turrets don't just snap to where they should go, but I wasted way, way, way, way, way, way too much time on this already
		}
	}
}
...
void moveBullet(Bullet* b) {
	b->x += sin(b->angle) * b->velocity;
	b->y -= cos(b->angle) * b->velocity;
}

void drawBullet(Bullet b) {
	drawSprite(48, 32, 4, 1, 1, deg(b.angle));
}
...
void drawShip(Ship* s) {
	...
	for (int i = 0; i < s->parts.size(); i++) {
		...
		if (p.turret) {
			drawSprite(32, 128, 16, 16, p.x, p.y, 1, deg(p.angle + p.turret_angle + rad(90)));
			Print(to_str((int)deg(p.turret_angle - rad(90 * p.rotation))), p.x, p.y);
		}
	}
}
...
void drawStars(float x, float y, float w, float h) {
	...
	for (int i = dy; i < dy + h; i++) {
		for (int j = dx; j < dx + w; j++) {
			key = (randC(i*17 + 5, j*i) % 100000);
			if (key == 1) {
				size = 1 + (randC(i, j) % 5);
				color = (randC(j, i) % 5);
				drawSprite((int)(size * size + size - 1) / 2, 0, size, size, j, i, 1, 0.0f, colors[color]);
			}
		}
	}
}
...
int main() {
	...
	float mX, mY;
	window.setView(view);
	auto shape = sf::RectangleShape{ sf::Vector2f{ view.getSize() } };
    ...
    while (window.isOpen()) {
		...
		if (elapsedTime >= targetTime) {
			...
			if (sf::Mouse::isButtonPressed(sf::Button::Left)) {
				for (Part p : player.parts) {
					if (p.turret) {
						Bullet b(p.x, p.y, p.angle + p.turret_angle + rad(90), hypot(p.dx, p.dy) + 50);
					}
				}
			}
		...
		shader.setUniform("offset", sf::Glsl::Vec2{ shape.getPosition() });
		...
		Print(to_str((int)deg(0 - atan2(mY - player.cy, mX - player.cx))), view.getCenter().x, view.getCenter().y - view.getSize().y * .4);
        window.display();
    }
