...
void targetGuns(Ship* s, float x, float y) {
	for (Part& p : s->parts) {
		if (p.turret) {
			p.turret_angle = atan2((p.y - y), (p.x - x)) - rad(90 * p.rotation) - p.angle;
			float ape = 0;
		}
	}
}
...
void drawShip(Ship* s) {
	...
	for (int i = 0; i < s->parts.size(); i++) {
		...
		if (p.turret) {
			drawSprite(32, 128, 16, 16, p.x, p.y, 1, 90 * p.rotation + deg(p.angle) + deg(p.turret_angle));
		}
	}
}
...
int main() {
    ...
    while (window.isOpen()) {
		...
		if (elapsedTime >= targetTime) {
			...
			targetGuns(&player, sf::Mouse::getPosition().x, sf::Mouse::getPosition().y);
