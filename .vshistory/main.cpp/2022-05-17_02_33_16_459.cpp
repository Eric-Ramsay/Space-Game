    ...
    while (window.isOpen()) {
        ...
        if (elapsedTime >= targetTime) {
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
				things.push_back(new Ship(safeC(player.x + cos(rad(player.angle)) * player.velocity, 1920), safeC(player.y + sin(rad(player.angle)) * player.velocity, 1080), player.angle, 50, 224, 16, 4, 1));
			}
