							...
							if ((!u || !d) && (!l || !r)) {
								...
								if (!found) {
									...
									if (!found) {
										...
										ape.v = 2 * calcV(ape.pos, x, y, ape.num) * max(0, ((ape.g - ape.h)/tot_dist));
										ape.f = cur_fuel - copy.fuel;
										ape.h = ape.h + .75 * ape.g + ape.v + ape.f;
										open.push_back(ape);
									}
								}
							}
...
int main() {
	...
	while (window.isOpen()) {
		...
		if (player != nullptr && !in_shipyard) {
			...
			for (int i = 0; i < 20; i++) {
				moveShip(&copy, up, down, left, right);
			}
			float g = dist(copy.pos.x, copy.pos.y, mX, mY);
			float h = dist(mX, mY, copy.pos.x, copy.pos.y);
			float v = calcV(copy.pos, mX, mY);
			float f = (cur_fuel - copy.energy);
			float total = .5 * h + .5 * v + .1 * g + .05 * f;

			//Print(to_str(player->path.size()), player->pos.x - 50, player->pos.y);

			//Print(to_str((int)(g)), player->pos.x + 50, player->pos.y - 50);
			//Print("Dist: " + to_str((int)(h)), player->pos.x + 50, player->pos.y - 25);
			//Print("Vel Dist: " + to_str((int)(v)), player->pos.x + 50, player->pos.y);
			//Print("Fuel Usage: " + to_str((int)(f)), player->pos.x + 50, player->pos.y + 25);
		}
		window.display();
	}
