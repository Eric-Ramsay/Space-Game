...
bool planetaryCollision(Ship* s, Planet planet) {
	float d = dist(planet.x, planet.y, s->pos.x, s->pos.y);
	if (d < (planet.size + 16 * s->parts.size())) {
	if (overlap(s->pos.x, s->pos.y, 16 * s->parts.size(), 16 * s->parts.size(), plan.x, plan.y, 5 * plan.size, 5 * plan.size)
		for (Part p : s->parts) {
			float x = partX(s->pos, p);
			float y = partY(s->pos, p);
			float dis = dist(partX(s->pos, p), partY(s->pos, p), planet.x, planet.y);
			if (dis < planet.size) {
				return true;
			}
		}
...
void applyGravity(Position& pos, Planet plan) {
	if (overlap(pos.x, pos.y, 5 * plan.size, 5 * plan.size, plan.x, plan.y, 5 * plan.size, 5 * plan.size)) {
