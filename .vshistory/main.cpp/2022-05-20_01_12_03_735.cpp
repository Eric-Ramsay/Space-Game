...
void moveShip(Ship* s, float dvv, float dhv, float da, int r) {
	...
	//First, calculate center of mass and center of thrust
	for (Part& p : s->parts) {
		//p.vv += dvv;
		//p.hv += dhv;
		p.x += sin(p.angle) * p.vv;
		p.y -= cos(p.angle) * p.vv;
		p.x += cos(p.angle) * p.hv;
		p.y += sin(p.angle) * p.hv;
		mbot += p.mass;
		mx_top += p.mass * p.x;
		my_top += p.mass * p.y;
		...
		if (p.thrust > 0 && p.rotation == r) {
			...
			tbot += p.thrust;
			switch (r) {
			case 0: d_vert -= p.thrust; break;
			case 1: d_hori -= p.thrust; break;
			case 2: d_vert += p.thrust; break;
			case 3: d_hori += p.thrust; break;
			}
		}
	}
	mx_top /= mbot;
	...
	float angle = atan2(mx_top - tx_top, my_top - ty_top);
	...
	float d;
	for (Part& p : s->parts) {
		angle = atan2(p.y - my_top, p.x - mx_top);
		d = dist(p.x, p.y, mx_top, my_top);
		//float x_v = cos(p.angle) * p.velocity;
		//float y_v = sin(p.angle) * p.velocity;
		//float x_d = cos(angle);
		//float y_d = sin(angle);
		//float ape = 0;
		/*
		...
		if (da != 0) {
			float c = cos(da);
			float s = sin(da);
			float x_dif = p.x - mx_top;
			float y_dif = p.y - my_top;
			p.angle += da;
			p.x = c * x_dif - s * y_dif + mx_top;
			p.y = s * x_dif + c * y_dif + my_top;
		}
		
		//OBJECTIVE: Given an angle, make objects rotate around a point
		/*angle = atan2(p.y - my_top, p.x - mx_top);
		p.angle += .0001;
		d = dist(mx_top, my_top, p.x, p.y);
		p.x = cos(angle) * d + mx_top;
		p.y = sin(angle) * d + my_top;*/
		/*/
		p.angle = -90 + (atan2(mousePos.y - (p.y + 8*4), mousePos.x - (p.x + 8*4))) * 180 / 3.14159265359;
		//p.angle = (angle * 180 / 3.14159265359);
		//p.angle = (angle * 180 / 3.14159265359);
		//p.x += sin(angle) * 1/100;
		//p.y += -cos(angle) * 1/100;
		//p.x = p.x * cos(angle) - p.y * sin(angle);
		//p.y = p.x * sin(angle) + p.y * cos(angle);
		float c = cos(rad(p.angle));
		float s = sin(rad(p.angle));
		float x_dif = p.x - mousePos.x;
		float y_dif = p.y - mousePos.y;
		p.x = cos(p.angle) * (p.x - mx_top) - sin(p.angle) * (p.y - my_top) + mx_top;
		p.y = sin(p.angle) * (p.x - mx_top) + cos(p.angle) * (p.y - my_top) + my_top;
		//p.x = c * x_dif - s * y_dif + mousePos.x;
		//p.y = s * x_dif + c * y_dif + mousePos.y;
		//p.x = cos(p.angle) * (p.x - mousePos.x) - sin(p.angle) * (p.y - mousePos.y) + mousePos.x;
		//p.y = sin(p.angle) * (p.x - mousePos.x) + cos(p.angle) * (p.y - mousePos.y) + mousePos.y;
		///p.x = mousePos.x + cos(rad(p.angle)) * 25;
		//p.y = mousePos.y + sin(rad(p.angle)) * 25;*/
		//Print(to_str(((int)deg(angle))), 1920 / 2, a++ * 80 + 1080 / 2, 4);
		if (p.crucial) {
			
			//Print(to_str(cos(p.angle)), 1920 / 2, a++ * 80 + 1080 / 2, 4);
			//Print(to_str(sin(p.angle)), 1920 / 2, a++ * 80 + 1080 / 2, 4);
			//Print(to_str((-cos(rad(p.angle))), 1920 / 2, a++ * 80 + 1080 / 2, 4);
			//Print(to_str((sin(angle))), 1920 / 2, a++ * 80 + 1080 / 2, 4);
		}
		//center of mass is the fulcrum on which thrust operates,
		//so i think rocket_thrust * dist from center of mass should be used to calc thrust
	}
...
void drawShip(Ship* s) {
	for (int i = 0; i < s->parts.size(); i++) {
		Part p = s->parts[i];
		drawSprite(p.sX + 16 * (p.rotation % 2), p.sY + 16 * (p.rotation / 2), 16, 16, p.x, p.y, 4, deg(p.angle));
	}
}
