...
void dockPos(Ship* s) {
	...
	double mx = s->pos.x * s->mass;
	double my = s->pos.y * s->mass;
	float mass = s->mass;
	float dx = fX(s->pos, 0) * s->mass;
	float dy = fY(s->pos, 0) * s->mass;
	float da = s->pos.da * mass;

	for (Ship* a : s->connected) {
		mx += a->pos.x * a->mass;
		my += a->pos.y * a->mass;
		dx += fX(a->pos, 0) * a->mass;
		dy += fY(a->pos, 0) * a->mass;
		da += a->pos.da * a->mass;
		mass += a->mass;
	}
	if (mass == 0) {
	...
	s->pos.dx = fX(Position(0, 0, 0, dx, dy), s->pos.a);
	s->pos.dy = fY(Position(0, 0, 0, dx, dy), s->pos.a);
	s->pos.da = da;
	s->x = (float)mx;
	s->y = (float)my;
	s->e_mass = mass;
	...
	for (Ship* a : s->connected) {
		a->pos.dx = fX(Position(0, 0, 0, dx, dy), a->pos.a);
		a->pos.dy = fY(Position(0, 0, 0, dx, dy), a->pos.a);
		a->pos.da = da;
		a->x = (float)mx;
		a->y = (float)my;
		a->e_mass = mass;
...
void updatePosition(Position& pos, float cx, float cy) {
	pos.a += pos.da;
	...
	if (cx != pos.x || cy != pos.y) {
		float co = cos(pos.da);
		float si = sin(pos.da);
		float x_dif = pos.x - cx;
		float y_dif = pos.y - cy;
		pos.x = co * x_dif - si * y_dif + cx;
		pos.y = si * x_dif + co * y_dif + cy;
	}
}


void force(Ship* s, Position f, bool recurs = true) {
	float dx = fX(f, s->pos.a);
	float dy = fY(f, s->pos.a);
...
void collide(Ship* a, Ship* b) {
	...
	//Position f = (a->pos * a->mass);
	//float pos_angle = atan2(a->pos.y - b->pos.y, a->pos.x - b->pos.x);
	//float dir_angle = atan2(fY(f, 0), fX(f, 0));
	//f.a = rad(90) + atan2(a->pos.y - b->pos.y, a->pos.x - b->pos.x);
	...
	aF.dx = fX(aF, 0); // sin(aF.a)* aF.dy + cos(aF.a) * aF.dx;
	aF.dy = fY(aF, 0); // cos(aF.a)* aF.dy - sin(aF.a) * aF.dx;

	bF.dx = fX(bF, 0); // sin(bF.a)* bF.dy + cos(bF.a) * bF.dx;
	bF.dy = fY(bF, 0); // cos(bF.a)* bF.dy - sin(bF.a) * bF.dx;

	aF.a = 0; bF.a = 0;

	float AB = atan2(a->pos.y - b->pos.y, b->pos.x - a->pos.x);
	float BA = atan2(b->pos.y - a->pos.y, a->pos.x - b->pos.x);

	float pos_angle = atan2(a->pos.y - b->pos.y, b->pos.x - a->pos.x); //a & b flipped for y bc it needs to be -1 * normal for the angles to work right
	float dir_angle_a = atan2(aF.dy, aF.dx);
	float dir_angle_b = atan2(bF.dy, bF.dx);

	float strike_force_a = (rad(180) - abs(abs(pos_angle) - abs(dir_angle_a))) / rad(180);
	float strike_force_b = (rad(180) - abs(-pos_angle - dir_angle_b)) / rad(180);
	aF = aF * strike_force_a;
...
bool future_collision(Ship* a_src, Ship* b_src, int steps = 0) {
	...
	int i = 0;
	do {
		updatePosition(a.pos, a.x, a.y);
		updatePosition(b.pos, b.x, b.y);
		for (int i = 0; i < a.parts.size(); i++) {
			Part pa = a.parts[i];
			a_src->parts[i].colliding = false;
			for (int j = 0; j < b.parts.size(); j++) {
				Part pb = b.parts[j];
				if (bounded(pa, a_src->pos, pb, b_src->pos)) {
					a_src->parts[i].colliding = true;
	...
	for (Ship* a : ships) {
		...
		for (Ship* b : ships) {
			if (a != b) {
				...
				if (true) {
					int max_dist = a->parts.size() * 16 + b->parts.size() * 16;
...
void moveBullet(Bullet* b) {
	updatePosition(b->pos, b->pos.x, b->pos.y);
	b->power--;
}
						...
						for (int r = 0; r < 2; r++) {
							if ((!u || !d) && (!l || !r)) {
								...
								for (int i = 0; i < num; i++) {
									...
									updatePosition(copy.pos, copy.x, copy.y);
