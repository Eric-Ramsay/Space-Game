...
void dockPos(Ship* s) {
	...
	float mass = s->mass;
	float dx = (sin(s->pos.a) * s->pos.dy + cos(s->pos.a) * s->pos.dx) * s->mass;
	float dy = (sin(s->pos.a) * s->pos.dx - cos(s->pos.a) * s->pos.dy) * s->mass;
	float da = s->pos.da * mass;

	for (Ship* ship : ship->connected) {
		mx += ship->pos.x * ship->mass;
		my += ship->pos.y * ship->mass;
		dx += (sin(ship->pos.a) * ship->pos.dy + cos(ship->pos.a) * ship->pos.dx) * ship->mass;
		dy += (sin(ship->pos.a) * ship->pos.dx - cos(ship->pos.a) * ship->pos.dy) * ship->mass;
		da += ship->pos.da * ship->mass;
		mass += ship->mass;
	}
	if (mass == 0) {
		return;
	...
	ship->pos.dx = fX(dx, ship->pos.a);
	ship->pos.dy = fY(dy, ship->pos.a);
	ship->pos.da = da;
	ship->x = (float)mx;
	ship->y = (float)my;
	ship->e_mass = mass;
	...
	for (Ship* s : ship->connected) {
		s->pos.dx = fX(dx, s->pos.a);
		s->pos.dy = fY(dy, s->pos.a);
		s->pos.da = da;
		s->x = (float)mx;
		s->y = (float)my;
		s->e_mass = mass;
