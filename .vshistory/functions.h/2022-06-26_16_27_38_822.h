...
bool bounded(Part p, Position ship, float x, float y, float w = 1, float h = 1) {
	...
	Point D = part.bl;
	sf::Vertex AD[2];
	AD[0].position = sf::Vector(A.x, A.y);
	AD[1].position = sf::Vector(D.x, D.y);
	sf::Vertex CB[2];
	CB[0].position = sf::Vector(C.x, C.y);
	CB[1].position = sf::Vector(B.x, B.y);
	sf::Vertex BA[2];
	BA[0].position = sf::Vector(B.x, B.y);
	BA[1].position = sf::Vector(A.x, A.y);
	sf::Vertex DC[2];
	DC[0].position = sf::Vector(D.x, D.y);
	DC[1].position = sf::Vector(C.x, C.y);
	window.draw(AD);
	window.draw(CB);
	window.draw(BA);
	window.draw(DC);
