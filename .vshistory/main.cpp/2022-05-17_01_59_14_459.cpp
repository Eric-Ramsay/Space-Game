...
template <typename Type> Type safeC(const Type& a, const Type& MAX = MAPSIZE) {
	if (a >= MAX) {
		return a % MAX;
	}
	else if (a < 0) {
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (elapsedTime >= targetTime) {
			...
			player.x = fmod(player.x + cos(player.angle) * player.velocity, 400);
			player.y = fmod(player.y + sin(player.angle) * player.velocity, 400);
