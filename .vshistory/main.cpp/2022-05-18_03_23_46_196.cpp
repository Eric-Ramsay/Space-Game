...
void moveShip(Ship* s, int r) {
	...
	int a = 0;
	for (Part& p : s->parts) {
		angle = atan2(my_top - p.y, mx_top - p.x);
		//p.angle += (angle * 180 / 3.14159265359)/1000;
		p.x += cos(rad(p.angle)) * 1/100;
		p.y += sin(rad(p.angle)) * 1/100;
		Print(to_str(90 + (angle * 180 / 3.14159265359)), 1920 / 2, a++ * 80 + 1080 / 2, 4);
