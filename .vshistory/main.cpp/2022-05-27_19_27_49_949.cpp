...
void targetGuns(Ship* s, float x, float y) {
	for (Part& p : s->parts) {
		if (p.turret) {
			p.turret_angle = atan2((p.y - y), (p.x - x));
			p.turret_angle += rad(90) * (1 + p.rotation);

			//angle can't be greater than += 45 degrees of its orientation
			if (p.turret_angle > rad(45 + 90 * (1 + p.rotation))) {
				p.turret_angle = rad(45 + 90 * (1 + p.rotation));
			}
			else if (p.turret_angle < rad(-45 + 90 * (1 + p.rotation))) {
				p.turret_angle = ad(-45 + 90 * (1 + p.rotation));
			}
		}
	}
}
...
void drawShip(Ship* s) {
	...
	for (int i = 0; i < s->parts.size(); i++) {
		...
		if (p.turret) {
			drawSprite(32, 128, 16, 16, p.x, p.y, 1, deg(p.turret_angle - rad(90) * p.rotation - p.angle));
		}
	}
}
