...
bool bounded(Part p, Position ship, float x, float y, float w = 1, float h = 1) {
	...
	short area = 2 * (p.w + p.h)
	return inTri(Point(x, y), part.tl, part.tr, part.br) || inTri(Point(x, y), part.tl, part.bl, part.br) || inTri(Point(x, y), part.bl, part.br, part.tr) || inTri(Point(x, y), part.br, part.tr, part.tl);
}
