...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
			...
			else if (event.type == sf::Event::MouseMoveEvent::x || event.type == sf::Event::MouseMoveEvent::y) {
				mX = player->x + sf::Mouse::getPosition().x * zoom - view.getSize().x / 2;
				mY = player->y + sf::Mouse::getPosition().y * zoom - view.getSize().y / 2;
			}
