...
void moveShip(Ship* s, int r) {
	...
	for (Part& p : s->parts) {
		...
		//angle = atan2(mousePos.y - my_top, mousePos.x - mx_top);
		...
		p.x = cos(p.angle) * (p.x - mx_top) - sin(p.angle) * (p.y - my_top) + mousePos.x;
		p.y = sin(p.angle) * (p.x - mx_top) + cos(p.angle) * (p.y - my_top) + mousePos.y;
