...
int main() {
    ...
    while (window.isOpen()) {
		...
		if (player != nullptr && !in_shipyard) {
			Print(to_str((int)player->pos.dx), view.getCenter().x - 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
			Print(to_str((int)player->pos.dy), view.getCenter().x + 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
