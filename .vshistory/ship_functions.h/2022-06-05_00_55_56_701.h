	...
	for (Part& p : s->parts) {
		...
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				...
				float x_force = -sin(angle) * dxt * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y) / 2.0;
				float y_force = -cos(angle) * dyt * dist(p.pos.x, p.pos.y, s->pos.x, s->pos.y) / 2.0;
				T += x_force;
				T += y_force;
				da += T / (5000 * s->e_mass);
