...
bool bounded(Part p1, Position ship1, Part p2, Position ship2) {
	...
	if (tl || tr || bl || br) {
		...
		bool tl = bounded(p2, ship2, part.tl.x, part.tl.y);
		bool tr = bounded(p2, ship2, part.tr.x, part.tr.y);
		bool bl = bounded(p2, ship2, part.bl.x, part.bl.y);
		bool br = bounded(p2, ship2, part.br.x, part.br.y);
	}
}
