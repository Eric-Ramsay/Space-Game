...
struct Part {
	...
	Part(std::string n = "Nothing", int dura = 25, Connection c = Connection(true, false, false, false), bool crucial = false) {
		name = n;
		durability = dura;
	}
};
	...
	if (command > -1) { //1 command module, check if everything is connected to it
		...
		while (open_list.size() > 0) {
			...
			for (int i = 0; i < 4; i++) {
				int cons = 0;
				switch (i) {
				case 0: i = safeC(current + 18, shipyard.size()); break; //current part is above considered part
				case 1: i = safeC(current - 1, shipyard.size()); break; //current part is to the right of the considered
				case 2: i = safeC(current - 18, shipyard.size()); break; //current part is below the considered
				case 3: i = safeC(current + 1, shipyard.size()); break; //current part is to the left of the considered
				}
...
std::vector<Part> initParts() {
	...
	parts.push_back(Part("Command Module", 25, all, true));
	parts.push_back(Part("Large Thruster", 30));
	parts.push_back(Part("Fuel Cell", 20, all));
	parts.push_back(Part("Weapons Platform", 35));
	parts.push_back(Part("Cargo Cell", 25, all));
	parts.push_back(Part("Armor Plating", 250));
	parts.push_back(Part("Small Thruster", 20));
	parts.push_back(Part("Shield Generator", 15));
	parts.push_back(Part("Radiator", 30, Connection(true, false, true, false)));
	parts.push_back(Part("Solar Panel", 25, all));
