...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (elapsedTime >= targetTime) {
			...
			player.x += cos(player.angle) * player.velocity;
			player.y += sin(player.angle) * player.velocity;
        }
        ...
        window.clear();
		Print(to_str((int)angle), 1920 / 2, 1080 / 2, 5);
		drawSprite(240, 16, 14, 13, x, y, 4, angle);
        window.display();
    }
