...
Connection rotConnect(Connection co, rotation r) {
	Connection c = co;
	switch (r) {
	case 1: c.top = co.left; c.right = co.top; c.bot = co.right; c.left = co.bot;
	case 2: con.top = c.left; con.bot = c.right;
	case 3:
	}
	return c;
}
...
bool connectedParts(Part a, Part b, int relation) {
	Connection a;
	Connection b;
	if (a.rotation == 0) { //connected to something below
		a_top = 
	}
	else if (a.rotation == 1) { //connected to something above
		
	}
	else if (a.rotation == 2) { //connected to something on the left

	}
	else if (a.rotation == 3) { //connected to something on the right
