    ...
    while (window.isOpen()) {
        ...
        if (elapsedTime >= targetTime) {
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
				...
				index = safeC(index + 25, shipyard.size());
			}
			accelerated = false;
		...
		if (in_shipyard) {
			for (int i = 0; i < shipyard.size(); i++) {
				if (shipyard[i].index > 0) {
					drawSprite(shipyard[i].sX + 16 * (shipyard[i].rotation % 2), shipyard[i].sY + 16 * (shipyard[i].rotation / 2), 16, 16, 50 + (i % 25) * 16*4, 50 + (i / 25) * 16 * 4, 4);
				}
				if (i == index) {
