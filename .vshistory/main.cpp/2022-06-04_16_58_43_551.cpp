...
int main() {
    ...
    while (window.isOpen()) {
		...
		bool right = false;
		if (player != nullptr) {
			mX = player->pos.x + sf::Mouse::getPosition().x * zoom - view.getSize().x / 2;
			mY = player->pos.y + sf::Mouse::getPosition().y * zoom - view.getSize().y / 2;
		...
		if (player != nullptr && !in_shipyard) {
			Print(to_str((int)player->pos.x), view.getCenter().x - 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
			Print(to_str((int)player->pos.y), view.getCenter().x + 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
			if (player->docked_at != nullptr) {
				Print("docked", view.getCenter().x, view.getCenter().y - view.getSize().y / 2.0 + 40, 3);
			}
		}
        window.display();
    }
