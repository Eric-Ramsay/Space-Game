    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				...
				if (event.mouseButton.button == sf::Mouse::Left) {
					...
					mouseForce.x = mX;
					mouseForce.y = mY;
					float angle = atan2(mY - player->y, mX - player->x) - player->pos.a;
					mouseForce.dx = 25 * -cos(angle);
					mouseForce.dy = 25 * sin(angle);
		...
		if (inGame) {
			if (!in_shipyard) {
				...
				down = sf::Keyboard::isKeyPressed(sf::Keyboard::W);
				left = sf::Keyboard::isKeyPressed(sf::Keyboard::W);
