...
void updatePosition(Position& pos) {
	...
	pos.y += cos(pos.a) * pos.dy - sin(pos.a) * pos.dx ;
	if (abs(pos.dx) < .001) {
		pos.dx = 0;
	}
	if (abs(pos.dy) < .001) {
...
void force(Ship* s, Position f) {
	...
	float x_force = sin(angle) * d * dx;
	float y_force = cos(angle) * d * dy;
	if (abs(x_force) < .001) {
		x_force = 0;
	}
	if (abs(y_force) < .001) {
...
void collide(Ship* a, Ship* b) {
	...
	aF.dy = sin(aF.a) * aF.dx - cos(aF.a) * aF.dy;

	bF.dx = sin(bF.a) * bF.dy + cos(bF.a) * bF.dx;
	bF.dy = sin(bF.a) * bF.dx - cos(bF.a) * bF.dy;

	float a_Angle = atan2(aF.dy, aF.dx);
	float b_Angle = atan2(bF.dy, bF.dx);

	float collision_angle = a_Angle - b_Angle;

	aF.a = 0; bF.a = 0;
	
	aF.dx = 0;// sin(collision_angle)* aF.dx;
	aF.dy = sin(collision_angle) * aF.dy;

	bF.dx = 0;// sin(collision_angle)* bF.dx;
	bF.dy = sin(collision_angle) * bF.dy;
