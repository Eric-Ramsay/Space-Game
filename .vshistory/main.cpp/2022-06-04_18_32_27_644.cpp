...
int main() {
    ...
    while (window.isOpen()) {
		...
		if (player != nullptr && !in_shipyard) {
			float angle = atan2(player->y - mY, player->x - mX) + player->parts[0].pos.angle;
			Print(to_str((int)(100 * cos(angle))), view.getCenter().x - 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
			Print(to_str((int)(100 * sin(angle))), view.getCenter().x + 50, view.getCenter().y - view.getSize().y / 2.0 + 20);
