...
bool bounded(Part p, Position ship, float x, float y, float w = 1, float h = 1) {
	...
	short area = (p.w * p.h);
	Point pt = Point(x, y);
	if (2 * triArea(part.tl, pt, part.bl) > area) {
		return true;
	}
	if (2 * triArea(part.bl, pt, part.br) > area) {
		return true;
	}
	if (2 * triArea(part.br, pt, part.tr) > area) {
		return true;
	}
	if (2 * triArea(part.tl, pt, part.tr) > area) {
		return true;
	}
	return false;
	bool bounded = ;
	bounded = bounded || ( > area);
	bounded = bounded || (2 * triArea(part))
	return 2 * triArea()
