...
void collide(Ship* a, Ship* b) {
	float dx, dy, angle, si, co;
	float angle = b->pos.a - a->pos.a;
	float si = sin(angle);
	float co 
	float a_dx = sin(angle) * s->pos.dy * s->mass + cos(angle) * s->pos.dx * s->mass;
	float a_dy = cos(angle) * s->pos.dy * s->mass - sin(angle) * s->pos.dx * s->mass;
	
	angle = a->pos.b - b->pos.b;
	float a_dx = sin(angle) * b->pos.dy * b->mass + cos(angle) * b->pos.dx * b->mass;
	float a_dy = cos(angle) * b->pos.dy * b->mass - sin(angle) * b->pos.dx * b->mass;
