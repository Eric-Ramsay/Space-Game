...
void drawPlanet(Planet* p, sf::View& view) {
	...
	static sf::Shader atmosphere;
	if (init) {
		init = false;
		if (!planet.loadFromFile("planet.frag", sf::Shader::Fragment)) {
			std::cout << "There was an error loading a fragment shader" << std::endl;
		}
		if (!atmosphere.loadFromFile("atmosphere.frag", sf::Shader::Fragment)) {
			std::cout << "There was an error loading a fragment shader" << std::endl;
		}
	}
	atmosphere.setUniform("texture", sf::Shader::CurrentTexture);
