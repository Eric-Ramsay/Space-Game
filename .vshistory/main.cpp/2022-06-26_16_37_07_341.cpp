...
int main() {
	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				for (Part p : player->parts) {
					if (parts[p.i].crucial) {
						if (bounded(p, player->pos, mX, mY, 1, 1)) {
							Print("Bounded", mX, mY);
						}

					}
					Rect r = createRect(p, player->pos);
					Point P = Point(mX, mY);
					Point A = r.tl;
					Point B = r.tr;
					Point C = r.br;
					...
					sf::Vertex vertices[] =
					{
						...
						sf::Vertex(sf::Vector2f(D, D), sf::Color::Red),
						sf::Vertex(sf::Vector2f(C, C), sf::Color::Red),
						sf::Vertex(sf::Vector2f(B, B), sf::Color::Red)
					};
					// draw it
					window.draw(vertices, 4, sf::Quads);
					
					sf::Vertex AD[2];
					AD[0].position = sf::Vector2f(A.x, A.y);
					AD[1].position = sf::Vector2f(D.x, D.y);
					sf::Vertex CB[2];
					CB[0].position = sf::Vector2f(C.x, C.y);
					CB[1].position = sf::Vector2f(B.x, B.y);
					sf::Vertex BA[2];
					BA[0].position = sf::Vector2f(B.x, B.y);
					BA[1].position = sf::Vector2f(A.x, A.y);
					sf::Vertex DC[2];
					DC[0].position = sf::Vector2f(D.x, D.y);
					DC[1].position = sf::Vector2f(C.x, C.y);

					window.draw(AD, 2, sf::Quads);
					window.draw(CB, 2, sf::Quads);
					window.draw(BA, 2, sf::Quads);
					window.draw(DC, 2, sf::Quads);
				}
				drawPosition(mX, mY);
			}
		}
