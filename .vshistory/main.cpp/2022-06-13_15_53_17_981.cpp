...
int main() {
	...
	while (window.isOpen()) {
		...
		if (player != nullptr && !in_shipyard) {
			...
			ape.pos = copy.pos;
			float g = dist(a.pos.x, a.pos.y, ape.pos.x, ape.pos.y);
			float h = dist(x, y, copy.pos.x, copy.pos.y);
			float v = calcV(ape.pos, x, y, ape.iterations);
			float f = (cur_fuel - copy.energy);
			float total = .5 * ape.h + .5 * ape.v + .1 * ape.g + .05 * ape.f;

			Print(to_str((int)(player->pos.x)), player->pos.x - 50, player->pos.y);
			Print(to_str((int)(player->pos.y)), player->pos.x + 50, player->pos.y);
		}
		window.display();
	}
