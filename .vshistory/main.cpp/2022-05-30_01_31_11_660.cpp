...
void moveShip(Ship* s, bool up = false, bool down = false, bool left = false, bool right = false) {
	...
	for (Part& p : s->parts) {
		p.pos.a += p.pos.da;
		p.pos.x += sin(p.pos.a) * p.pos.dy;
		p.pos.y -= cos(p.pos.a) * p.pos.dy;
		p.pos.x += cos(p.pos.a) * p.pos.dx;
		p.pos.y += sin(p.pos.a) * p.pos.dx;
		mass += p.mass;
		mx += p.mass * p.pos.x;
		my += p.mass * p.pos.y;
	}
...
void moveBullet(Bullet* b) {
	b->pos.x += cos(b->angle) * b->velocity;
	b->pos.y += sin(b->angle) * b->velocity;
	b->power--;
}
    ...
    while (window.isOpen()) {
		...
		if (elapsedTime >= targetTime) {
			...
			if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
				for (Part p : player.parts) {
					if (p.turret) {
						Bullet* b = new Bullet(p.pos.x, p.pos.y, p.turret_angle + rad(90), hypot(p.pos.dx, p.pos.dy) + 10);
						b->pos.x += cos(b->angle) * 6;
						b->pos.y += sin(b->angle) * 6;
						b->next = bullets;
						bullets = b;
					}
				}
