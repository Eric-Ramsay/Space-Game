	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				Print(to_str((rad(180) - abs(abs(a) - abs(a1)))/rad(180)), player->pos.x - 200, player->pos.y - 100);
				Print(to_str((rad(180) - abs(-abs(a) - abs(a1)))/rad(180)), player->pos.x - 200, player->pos.y - 50);
			}
		}
		window.display();
	}
