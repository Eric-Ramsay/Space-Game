...
void collide(Ship* a, Ship* b) {
	float angle = a->pos.a - b->pos.a;
	float dx = sin(angle) * b->pos.dy * b->mass + cos(angle) * b->pos.dx * b->mass;
	float dy = cos(angle) * b->pos.dy * b->mass - sin(angle) * b->pos.dx * b->mass;
	float da = b->pos.da * b->mass;
	a->pos.dx += dx;
	a->pos.dy += dy;
	a->pos.da += da;
	b->pos.dx -= dx;
				...
				for (Part pb : b->parts) {
					if (overlap(pa.pos, 8, pb.pos, 8)) {
						if (pa.name == "Docking Hatch" && pb.name == "Docking Hatch") {
							...
							std::vector<Ship*> a_connections = a->connected;
							std::vector<Ship*> b_connections = b->connected;
							link(a, b);
							link(b, a);
							//Calculate Destruction of Parts
							//Disconnect Parts into separate ships, etc. . .
							//Blah
							//calculateMass(a);
							//calculateMass(b);
							//dockPos(a);
							//dockPos(b);
							//a->connected = a_connections;
							//b->connected = b_connections;
						}
					}
				}
			}
