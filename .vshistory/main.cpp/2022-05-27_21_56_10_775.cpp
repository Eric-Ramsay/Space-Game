...
void targetGuns(Ship* s, float x, float y) {
	for (Part& p : s->parts) {
		if (p.turret) {
			...
			float turret_angle = deg(p.turret_angle - rad(90 * p.rotation));

			if (p.turret_angle < angle) {
				if (turret_angle < 330) {
					p.turret_angle += .1;
				}
				else {
					p.turret_angle = rad(330);
				}
			}
			else if (p.turret_angle > angle) {
				if (turret_angle > 210) {
					p.turret_angle -= .1;
				}
			}
			if ((int)deg(p.turret_angle - rad(90 * p.rotation)) > 330) {
				float ape = 0;
			}
			p.turret_angle = ((int)(10 * p.turret_angle)) / 10.0;
		}
	}
}
