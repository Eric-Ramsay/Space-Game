...
int main() {
	...
	while (window.isOpen()) {
			...
			for (Ship* s : ships) {
				if (s == player) {
					if (player->path.size() > 0) {
						...
						player->path[0].iterations--;
						if (player->path[0].iterations <= 0) {
		...
		if (player != nullptr && !in_shipyard) {
			...
			float v = calcV(copy.pos, mX, mY, 20);
			float f = (cur_fuel - copy.energy);
			float total = .5 * h + .5 * v + .1 * g + .05 * f;

			Print(to_str((int)(total)), player->pos.x - 50, player->pos.y);

			Print(to_str((int)(g)), player->pos.x + 50, player->pos.y - 50);
			Print(to_str((int)(h)), player->pos.x + 50, player->pos.y - 25);
			Print(to_str((int)(v)), player->pos.x + 50, player->pos.y);
			Print(to_str((int)(f)), player->pos.x + 50, player->pos.y + 25);
		}
		window.display();
	}
