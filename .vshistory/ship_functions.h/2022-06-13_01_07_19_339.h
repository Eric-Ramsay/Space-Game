		...
		for (Ship* b : ships) {
			if (a != b) {
				...
				if (can_collide) {
					...
					max_dist *= 16;
					if (overlap(a->pos, max_dist / 2, b->pos, max_dist / 2)) {
...
float partX(Position pos, Part p) {
	float co = cos(pos.a);
	float si = sin(pos.a);
	return co * p.pos.x - si * p.pos.y + pos.x;
	dY = si * p.pos.x + co * p.pos.y + s->pos.y;
}

float partY(Position pos, Part p) {
	float co = cos(pos.a);
	float si = sin(pos.a);
	return si * p.pos.x + co * p.pos.y + s->pos.y;
}
	...
	for (Part& p : s->parts) {
		if (p.thrust > 0) {
			...
			if (p.rotation == 0 && up || p.rotation == 1 && right || p.rotation == 2 && down || p.rotation == 3 && left) {
				...
				thrust.x = s->pos.x + p.pos.x;
				thrust.y = s->pos.y + p.pos.y;
				force(s, thrust);
			}
		}
	}
...
std::deque<PathPos> path(Ship a, float x, float y) {
	...
	Position final;
	Ship current = a;
	current.p = PathPos(a.pos);
	std::deque<Ship> open = { current };
	std::vector<Ship> closed = {};
	final.x = x;
	final.y = y;
	while (open.size() > 0 && !stop) {
		int min = 0;
		for (int i = 1; i < open.size(); i++) {
			if (open[i].p.g + open[i].p.h < open[min].p.g + open[min].p.h) {
				min = i;
			}
		}
		current = open[min];
		...
		if (abs(current.pos.x - x) < 3 && abs(current.pos.y - y) < 3) {
			stop = true;
		}
		else {
			for (int u = 0; u < 2; u++) {
				for (int d = 0; d < 2; d++) {
					for (int l = 0; l < 2; l++) {
						for (int r = 0; r < 2; r++) {
							bool found = false;
							Ship ape = current;
							moveShip(&ape, u, d, l, r);
							updatePosition(ape.pos);
							ape.p = PathPos(ape.pos, closed.size() - 1, u, d, l, r);
							for (Ship s : closed) {
								if (ape.p == s.p) {
									found = true;
								}
							...
							if (!found) {
								...
								if (!found) {
									ape.p.g = dist(a.pos.x, a.pos.y, ape.pos.x, ape.pos.y);
									ape.p.h = dist(x, y, ape.pos.x, ape.pos.y);
									open.push_back(ape);
								}
							}
						}
	...
	if (stop) {
		while (current.p.parent > -1) {
			path.push_front(current.p);
			current = closed[current.p.parent];
		}
	}
	return path;
}