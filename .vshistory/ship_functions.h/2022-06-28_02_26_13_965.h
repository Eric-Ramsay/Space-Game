...
void collide(Ship* a, Ship* b) {
	...
	float ADir = atan2(aF.dy, aF.dx);
	float BDir = atan2(bF.dy, bF.dx);
	...
	float multA = (rad(180) - abs(abs(AB) - abs(ADir))) / rad(180);
	float multB = (rad(180) - abs(abs(BA) - abs(BDir))) / rad(180);
