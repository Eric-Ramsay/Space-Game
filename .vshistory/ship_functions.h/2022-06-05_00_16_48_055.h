...
void collide(Ship* a, Ship* b, bool recursive = true) {
	...
	si = sin(angle);
	co = cos(angle);
	dx = si * b->pos.dy * b->mass + co * b->pos.dx * b->mass;
	dy = co * b->pos.dy * b->mass - si * b->pos.dx * b->mass;
	
	ax = (.5 * b->mass * (dx - a->pos.dx) + a->mass * a->pos.dx + b->mass * dx) / mass;
	ay = (.5 * b->mass * (dy - a->pos.dx) + a->mass * a->pos.dx + b->mass * dx) / mass;
