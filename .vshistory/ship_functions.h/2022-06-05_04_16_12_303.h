...
void collide(Ship* a, Ship* b) {
	...
	float angle = atan2(a->pos.y - b->pos.y, a->pos.x - b->pos.x) - (a->pos.a - b->pos.a);
	float dx = sin(angle) * (sin(angle) * b->pos.dy + cos(angle) * b->pos.dx);
	float dy = cos(angle) * (cos(angle) * b->pos.dy - sin(angle) * b->pos.dx);
