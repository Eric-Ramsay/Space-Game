...
void moveShip(Ship* s, int r) {
	...
	for (Part& p : s->parts) {
		...
		if (p.crucial) {
			...
			Print(to_str((cos(p.angle) * (p.x - mousePos.x) - sin(p.angle) * (p.y - mousePos.y) + mousePos.x)), 1920 / 2, a++ * 80 + 1080 / 2, 4);
			Print(to_str((p.y = sin(p.angle) * (p.x - mousePos.x) + cos(p.angle) * (p.y - mousePos.y) + mousePos.y)), 1920 / 2, a++ * 80 + 1080 / 2, 4);
