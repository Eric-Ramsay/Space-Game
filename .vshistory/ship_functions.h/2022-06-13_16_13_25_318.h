...
void evaluate(Ship s, PathPos& p, float x, float y) {
	Ship copy = s;
	s.pos = p.pos;
	float cur_fuel = copy.energy;
	for (int i = 0; i < 20; i++) {
		moveShip(&copy, p.up, p.down, p.left, p.right);
	}
	p.pos = copy.pos;
	p.iterations = 20;
	p.g = 0; // dist(a.pos.x, a.pos.y, ape.pos.x, ape.pos.y);
	p.h = dist(x, y, copy.pos.x, copy.pos.y);
	p.v = calcV(ape.pos, x, y, ape.iterations);
	p.f = 0;// (cur_fuel - copy.energy);
	p.h = ape.h + ape.v + ape.f;
}
...
std::deque<PathPos> path(Ship a, float x, float y) {
	...
	while (closed.size() < 100 && open.size() > 0 && !stop) {
		int min = 0;
