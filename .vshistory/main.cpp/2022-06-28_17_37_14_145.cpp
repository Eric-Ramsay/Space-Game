	...
	while (window.isOpen()) {
		...
		if (player != nullptr) {
			...
			if (!in_shipyard) {
				...
				Print(to_str(multX(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 125);
				Print(to_str(multY(player->pos, Position(mX, mY))), player->pos.x - 200, player->pos.y - 125);
			}
		}
		window.display();
	}
